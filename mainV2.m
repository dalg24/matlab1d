...
%     for k=1:NPAR.max_ss_iter,
        ...
        k=1;disp(sprintf('Steady-state N-TH iteration # %4i',k));

        % compute residula (the fission src term is fixed)
        [res,err2] = comp_residual( MSH, XS, NPAR, BC, therm, hydro, vec );
        if(NPAR.MatFree)
            ...
            J=compute_num_jacobian( res, MSH, XS, NPAR, BC, therm, hydro, vec);
        end
        ...
        for i=1:NPAR.MaxNewton
            if(NPAR.MatFree)
                [delta_vec,flag,relres,iter] = gmres( @comp_res_diff, -res, [],tol,maxit,[],[],[], ...
                    MSH, XS, NPAR, BC, therm, hydro, vec, res, epsi);
                if(NPAR.PreCond)
                    [delta_vec,kkk,ppp]=OS_ss(MSH, NPAR, XS, therm, hydro, BC, delta_vec, vec);
                end
            else
                delta_vec =-J\res;
            end
            ...
            if( err2 <tol)
                ...
                if(NPAR.MatFree)
                    [delta_vec,flag,relres,iter] = gmres( @comp_res_diff,-res,[],tol,maxit,[],[],[], MSH, XS, NPAR, BC, therm, hydro, vec, res, epsi);
                    if(NPAR.PreCond)
                        [delta_vec]=OS_prec_ss(MSH, NPAR, XS, therm, hydro, BC, delta_vec, vec);
                    end
                else
                    % compute the analytical Jacobian
                    freq=1;
...
my_TH_plot(fid+1, true, false, MSH, NPAR, therm, hydro, vec );
my_N_plot(fid+2, true, false, MSH, NPAR, XS, BC, therm, hydro, vec );


% [fl]=re_ordering(MSH,vec(1:NPAR.neu),NPAR.gn,XS.G,NPAR.dof);

if(MSH.dim==1)
%      save save_V2_ss.mat vec;
% else
%     save save2r.mat vec keff;
end

error('the rest is not yet modified, do not run it!!!')
